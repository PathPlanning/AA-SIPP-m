...
void VelocityConstraints::updateCellSafeIntervals(std::pair<int, int> cell)
{
    ...
    for(int k = 0; k < secs.size(); k++)
    {
        ...
        if(cls == 3)
        {
            ...
            interval.second = sec.g1 + sqrt(radius*radius - dist*dist) - ha;
        }
        else if(cls == 4)
        {
            interval.first = sec.g2 - sqrt(radius*radius - dist*dist) + sqrt(db - dist*dist);
    ...
    for(unsigned int i = 0; i < safe_intervals[curNode.i][curNode.j].size(); i++)
                ...
                && safe_intervals[curNode.i][curNode.j][i].first <= (curNode.Parent->interval.second + curNode.g - curNode.Parent->g))
        {
            ...
            for(auto it = range.first; it != range.second; it++)
                ...
                if((it->second.g + fabs(curNode.heading - it->second.heading)*tweight/180) - curNode.g < CN_EPSILON)//take into account turning cost
                {
                    has = true;
                    break;
                }

...
bool Instance::getInstance(const char *fileName, int num)
{
    XMLDocument doc;
    ...
    int k(0);
    for(element = root->FirstChildElement(CNS_TAG_AGENT); element; element = element->NextSiblingElement("agent"))
    {
        Agent agent;
    ...
    root = doc.FirstChildElement(CNS_TAG_ROOT)->FirstChildElement("tasks");

    for(element = root->FirstChildElement("task"); element; element = element->NextSiblingElement("task"))
    {
        Task task;
        task.i = element->IntAttribute("y");
        task.j = element->IntAttribute("x");
        task.goal_i = element->Int64Attribute("goal.y");
        task.goal_j = element->Int64Attribute("goal.x");
...
bool Instance::validateTask(const Map &map)
{
    LineOfSight los;
    for(Agent a:agents)
    {
...
Agent Instance::getAgent(unsigned int id) const
{
    if(id < agents.size())
        return agents[id];
    else
...
Task Instance::getTask(unsigned int id) const
{
    if(id < tasks.size())
        return tasks[id];
    else
...
unsigned int Instance::getNumberOfAgents() const
{
    return agents.size();
}

unsigned int Instance::getNumberOfTasks() const
{
    return tasks.size();
}

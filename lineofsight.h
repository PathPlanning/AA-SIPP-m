        ...
        if(delta_x > delta_y)
        {
            ...
            for(int n = 1; n <= extraCheck; n++)
            {
                ...
                for(k = 1; k <= num; k++)
                    ...
                    {
                        if(!map.CellIsStart(x1 - n*step_x, y1 + k*step_y))
                            return false;
                        double dist = fabs((x1-x2)*(y1 + k*step_y)+(y2-y1)*(x1 - n*step_x)+(x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                        if(dist < 2*agentSize)
                            return false;
                    }
                for(k = 1; k <= num; k++)
                    ...
                    {
                        if(!map.CellIsStart(x2 + n*step_x, y2 - k*step_y))
                            return false;
                        double dist = fabs((x1-x2)*(y2 - k*step_y)+(y2-y1)*(x2 + n*step_x)+(x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                        if(dist < 2*agentSize)
                            return false;
                    }
            }
            ...
            for(x = x1; x != x2 + step_x; x++)
            {
                ...
                {
                    if(!map.CellIsStart(x, y))
                        return false;
                    double dist = fabs((x1-x2)*y + (y2-y1)*x + (x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                    if(dist < 2*agentSize)
                        return false;
                }
                if(x < x2 - extraCheck)
                {
                    ...
                    for(k = 1; k <= num; k++)
                        ...
                        {
                            if(!map.CellIsStart(x, y + k*step_y))
                                return false;
                            double dist = fabs((x1-x2)*(y + k*step_y) + (y2-y1)*x + (x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                            if(dist < 2*agentSize)
                                return false;
                        }
                }
        ...
        else
        {
            ...
            for(int n = 1; n <= extraCheck; n++)
            {
                ...
                for(k = 1; k <= num; k++)
                    ...
                    {
                        if(!map.CellIsStart(x1 + k*step_x, y1 - n*step_y))
                            return false;
                        double dist = fabs((x1-x2)*(y1 - n*step_y) + (y2-y1)*(x1 + k*step_x) + (x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                        if(dist < 2*agentSize)
                            return false;
                    }
                for(k = 1; k <= num; k++)
                    ...
                    {
                        if(!map.CellIsStart(x2 - k*step_x, y2 + n*step_y))
                            return false;
                        double dist = fabs((x1-x2)*(y2 + n*step_y) + (y2-y1)*(x2 - k*step_x) + (x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                        if(dist < 2*agentSize)
                            return false;
                    }
            }
            ...
            for(y = y1; y != y2 + step_y; y += step_y)
            {
                ...
                if(y < y2 - extraCheck)
                {
                    ...
                    for(k = 1; k <= num; k++)
                        ...
                        {
                            if(!map.CellIsStart(x + k*step_x, y))
                                return false;
                            double dist = fabs((x1-x2)*y + (y2-y1)*(x + k*step_x) + (x2*y1-x1*y2))/sqrt(pow(x1-x2,2)+pow(y1-y2,2));
                            if(dist < 2*agentSize)
                                return false;
                        }
                }

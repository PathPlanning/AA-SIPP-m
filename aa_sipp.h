...
private:
    ...
    double MoveCost(int start_i, int start_j, int fin_i, int fin_j);
    ...
    std::vector<constraint> findConflictCells(Node cur);
    std::vector<std::pair<double, double> > findIntervals(Node curNode, std::vector<double> &EAT);
    std::vector<conflict> CheckConflicts();//brute force checker. It splits final(already built) trajectories into sequences of points and checks distances between them
